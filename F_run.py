...
from F_tray import KF_Keras, Return_model_T, Img_Prepare
...
class dp_window:
    def __init__(self, camera_image, Q):
        ...
        self.reject_limit = 0.95
        self.IMG_SIZE1, self.IMG_SIZE2 = 200, 200
        self.img_save_path = 'Reject_Images'
        self.camera_setting = './camera_settingN.pfs'
        self.weights = './trayF.h5'
        ...
        window_info = str("Nongshim Noksan Deep Learning Program")
        ...
        """ 배경에 들어갈 이미지 정의 """
        camera_image = cv2.resize(camera_image, (self.cam_x_size, self.cam_y_size))
    ...
    def load_network(self):    #신경망 구조 불러오기
       self.VAE = Return_model_T(self.IMG_SIZE1, self.IMG_SIZE2)
    ...
    def get_predict(self):    #카메라로부터 이미지를 불러와 예측값을 불러옴
        ...
        if self.cam_status == 'start':
            ...
            if ready:
                ...
                print("Detected : {}".format(detect)) # 검사추정치 출력하기 OK Reject
                
                self.decide_ng = 'ok'
                DATA = float(round(detect[0][0], 2))
                if DATA < self.reject_limit:
                    self.save_img(self.img)
                    self.reject_num += 1
                    self.decide_ng = 'ng'
                    error_data = ['Reject'] # 'Reject'
